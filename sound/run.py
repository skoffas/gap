...
# TODO: Modified the dataset to 16-bit mono, 44.1 kHz to apply inaudile sound.
SR = 44100
...
def poison(sample_path, trigger):
    ...
    mfccs = librosa.feature.mfcc(signal, sr, n_mfcc=40, n_fft=400,
                                 hop_length=160)
...
def apply_trigger(x_train, y_train, f_train, x_validation, y_validation,
                  f_validation, trigger, trojan_samples):
        ...
        for arch_name in ["trojaning_attacks", "adv_detection", "lstm_att"]:
            for epochs in [300]:
                ...
                for trojan_samples in [20, 40, 60, 80]:
                    # Size is a percentage in sound
                    #for size in [15, 30, 45, 60, 98]:
                    data.append(wrapper(data_clean, signal_test, partial,
